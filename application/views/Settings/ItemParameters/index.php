<?php 
$currentPage="Item Parameter";
$pageBack=base_url()."Settings/ItemParameters/";
$addPage=base_url()."Settings/ItemParameters/add/";
$deletePage=base_url()."Settings/ItemParameters/Delete/";
$addButton="Add";
?>
<!DOCTYPE html>
<html>
    ...
    <div id="main">
        ...
        <section id="content_wrapper">
            <header id="topbar">
                ...
                <?php  if (check_role_assigned('item_parameters','add')) {?>
                                ...
                                <div class="panel-body pn">
                                    <table class="table table-hover" id="datatable">
                                        ...
                                        <tbody>
                                            <?php 
                                            $i=1;
                                                foreach($ItemParameters as $post)
                                                {
                                            ?>
                                            <tr>
                                                <td><?=$i++?></td>
                                                <td><?=$post->subcategory?></td>
                                                <td><?=$post->parameter_name?></td>
                                                <td><?=$post->description?> </td>
                                                <td class="text-center">
                                            <?php 
                                                if (check_role_assigned('item_parameters', 'edit')) {
                                                    echo "<div class='btn-group'>".anchor($addPage . $post->id, '<i class="fa fa-edit"></i>', ['title'=>'Edit Item Parameter','class' => 'btn btn-warning btn-xs', 'style' => 'float:left; margin-right:5px; margin-top:1px;']);
                                                    echo '</div>';
                                                }
                                                if (check_role_assigned('item_parameters', 'delete')) {
                                                    echo '<div class="btn-group"><a href="#" onclick="javascript:deleteBox(' . $post->id . ')" title="Delete Item Parameter" class="btn btn-danger btn-xs cancel "><span class="fa fa-trash-o"></span></a>';
                                                    echo '</div>';
                                                }
                                                ?>
                                                </td>
