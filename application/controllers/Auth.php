...
class Auth extends CI_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
				...
				// 		$year =date('Y');
				// 	}
				// $res=$this->Queries->getSingleRecord('select * from '.TBL_FINANCIAL_YEAR.' where isdelete=0 order by id desc');
				// 		$dbyear=date_create_from_format('Y-m-d',$res->start_date);
				// 		$dbyear=date_create_from_format($dbyear,'Y');
				// 		if($year<=$dbyear)
				// 		{
				// 			$data=array(
				// 								'id'=>$res->id,
				// 								'name'=>$res->name,
				// 								'ms_value'=>$res->ms_value,
				...
				// 			$res1=$this->Queries->getSingleRecord('select * from '.TBL_FINANCIAL_YEAR.' where isdelete=0 and (start_date>="'.$start_date.'"   or end_date<="'.$end_date.'")');
				// 			if($res1!=null)
				// 			{
				// 				$data=array(
				// 								'id'=>$res1->id,
				// 								'name'=>$res1->name,
				// 								'ms_value'=>$res1->ms_value,
	...
	public function change_password()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->session->set_flashdata('error_msg', $this->data['message']);
			...
			// render
			$this->_render_page('Auth/profile', ['errordata' => $this->data, 'userdata' => $userdata]);
		} else {
			$identity = $this->session->userdata['logged_in']['userid'];
	...
	public function forgot_password()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($forgotten) {
				...
				$this->session->set_flashdata('success_msg', 'Check Your Email for Further Process');
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect("Auth/forgot_password", 'refresh'); //we should display a confirmation page here instead of the login page
			} else {
				$this->session->set_flashdata('error_msg', 'Mail Could Not Be Sent..');
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect("Auth/forgot_password", 'refresh');
			}
		}
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user) {
			// if the code is valid then display the password f form
